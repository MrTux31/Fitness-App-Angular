...
export class ExerciceService {
  ...
  deleteExercice(exercice: Exercice): Observable<Exercice> {
    return this.http.delete<Exercice>(this.exerciceAPI + '/' + exercice.id).pipe(
      tap(() => {
        // Supprime l'exercice du localStorage une fois la requête HTTP réussie
        this.deleteExerciceFromLocalStorage(exercice);
      })
    );;
  }
  ...
  deleteExerciceFromLocalStorage(exercice: Exercice) {
    // Récupération de l'objet exercices depuis le localStorage
    let exercices = JSON.parse(localStorage.getItem('exercices') || '{}');

    // Vérifie que la routine existe
    if (exercices[exercice.routineId]) {
      // Supprime l'exercice spécifique
      delete exercices[exercice.routineId][exercice.id];

      // Si après suppression la routine est vide, on peut aussi supprimer la routine
      if (Object.keys(exercices[exercice.routineId]).length === 0) {
        delete exercices[exercice.routineId];
      }
